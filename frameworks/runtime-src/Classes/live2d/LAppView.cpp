...
void LAppView::onEnter()
{
    ...
    // 画面の表示の拡大縮小や移動の変換を行う行列  用于放大和变换屏幕显示的放大和移动的矩阵。
    ...
    float bottom = -ratio;
    float top = ratio;
	//对应于设备的屏幕范围。X的左端，X的右端，Y的下端，Y的上端
    ...
    // 表示範囲の設定  显示范围的设定
    viewMatrix->SetMaxScale(ViewMaxScale); // 限界拡大率  边际放大率
    viewMatrix->SetMinScale(ViewMinScale); // 限界縮小率  边际缩小率
